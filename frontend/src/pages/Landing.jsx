    ...
    <div style={{ background: '#ffffff', color: '#1a1a2e', fontFamily: "'Space Grotesk', -apple-system, BlinkMacSystemFont, sans-serif", minHeight: '100vh' }}>
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.1rem 2.5rem', borderBottom: '1px solid #f0f0f8', position: 'sticky', top: 0, background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(12px)', zIndex: 100 }}>
        ...
        <div className="nav-links" style={{ display: 'flex', gap: 32, alignItems: 'center' }}>
          <a href="#features" style={{ color: '#6b6b8a', fontSize: 14, fontWeight: 500, textDecoration: 'none', transition: 'color 0.2s' }} onMouseOver={e => e.target.style.color='#534AB7'} onMouseOut={e => e.target.style.color='#6b6b8a'}>Features</a>
          <a href="#pricing" style={{ color: '#6b6b8a', fontSize: 14, fontWeight: 500, textDecoration: 'none', transition: 'color 0.2s' }} onMouseOver={e => e.target.style.color='#534AB7'} onMouseOut={e => e.target.style.color='#6b6b8a'}>Pricing</a>
          <a href="#faq" style={{ color: '#6b6b8a', fontSize: 14, fontWeight: 500, textDecoration: 'none', transition: 'color 0.2s' }} onMouseOver={e => e.target.style.color='#534AB7'} onMouseOut={e => e.target.style.color='#6b6b8a'}>FAQ</a>
          <button onClick={() => navigate('/auth')} style={{ background: 'none', border: '1.5px solid #534AB7', color: '#534AB7', padding: '8px 18px', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }} onMouseOver={e => { e.target.style.background='#534AB7'; e.target.style.color='#fff'; }} onMouseOut={e => { e.target.style.background='none'; e.target.style.color='#534AB7'; }}>Login</button>
          <button onClick={() => navigate('/auth')} className="btn-hover" style={{ background: '#534AB7', border: 'none', color: '#fff', padding: '8px 20px', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer', transition: 'all 0.2s', boxShadow: '0 4px 15px rgba(83,74,183,0.25)' }}>Get Started Free</button>
        </div>
      </nav>
      ...
      <section style={{ textAlign: 'center', padding: '6rem 2rem 5rem', maxWidth: 860, margin: '0 auto', position: 'relative' }}>
        ...
        <h1 className="hero-title" style={{ fontFamily: "'Syne', sans-serif", fontSize: 58, fontWeight: 800, lineHeight: 1.08, marginBottom: '1.5rem', letterSpacing: '-1.5px', color: '#1a1a2e' }}>
          Audit Your Website<br />Like a <span style={{ color: '#534AB7', position: 'relative' }}>Pro</span>
        </h1>
        <p style={{ fontSize: 17, color: '#6b6b8a', lineHeight: 1.8, marginBottom: '2.5rem', maxWidth: 560, margin: '0 auto 2.5rem', fontWeight: 400 }}>
          Get instant AI-powered analysis of your website's SEO, Performance, Security and Bugs. Fix issues faster and outrank your competitors.
        </p>
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => document.getElementById('features').scrollIntoView({ behavior: 'smooth' })} style={{ background: '#fff', color: '#1a1a2e', border: '1.5px solid #e2e2ef', padding: '15px 36px', borderRadius: 10, fontSize: 15, fontWeight: 600, cursor: 'pointer', transition: 'all 0.2s' }} onMouseOver={e => e.currentTarget.style.borderColor='#534AB7'} onMouseOut={e => e.currentTarget.style.borderColor='#e2e2ef'}>See How It Works</button>
        </div>
      ...
      <section style={{ borderTop: '1px solid #f0f0f8', borderBottom: '1px solid #f0f0f8', padding: '2.5rem 2rem', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', maxWidth: 860, margin: '0 auto', textAlign: 'center' }} className="grid-4">
        {[['500+', 'Websites Audited'], ['98/100', 'PageSpeed Score'], ['50+', 'Audit Factors'], ['Free', 'To Start']].map(([num, label]) => (
          <div key={label} style={{ padding: '0.5rem' }}>
            ...
            <div style={{ fontSize: 12, color: '#6b6b8a', marginTop: 5, fontWeight: 500, letterSpacing: '0.5px' }}>{label}</div>
          </div>
        ))}
      </section>
      ...
      <section id="features" style={{ padding: '6rem 2rem', maxWidth: 1040, margin: '0 auto' }}>
        ...
        <div className="grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem' }}>
          {[
            { icon: '◈', title: 'SEO Analysis', desc: 'Meta tags, headings, keywords, sitemap and 20+ SEO factors checked instantly.' },
            { icon: '⚡', title: 'Performance Audit', desc: 'Page speed, Core Web Vitals, image optimization and load time analysis.' },
            { icon: '🔒', title: 'Security Check', desc: 'HTTPS, security headers, vulnerabilities and SSL certificate detection.' },
            { icon: '🐛', title: 'Bug Detection', desc: 'JavaScript errors, broken links, console errors and code quality issues.' },
            { icon: '⚔', title: 'Competitor Analysis', desc: 'Compare your site against any competitor side by side with detailed scores.' },
            { icon: '📄', title: 'Export Reports', desc: 'Download professional PDF reports, CSV data or JSON for your clients.' },
          ].map(({ icon, title, desc }) => (
            <div key={title} className="card-hover" style={{ background: '#fff', border: '1.5px solid #f0f0f8', borderRadius: 14, padding: '1.75rem', transition: 'all 0.25s', cursor: 'default' }}>
              <div style={{ width: 44, height: 44, background: '#EEEDFE', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, marginBottom: '1.1rem' }}>{icon}</div>
      ...
      <section id="pricing" style={{ padding: '6rem 2rem', maxWidth: 1040, margin: '0 auto' }}>
        ...
        <div className="grid-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.25rem' }}>
          {[
            ...
            <div key={name} style={{ background: popular ? '#534AB7' : '#fff', border: popular ? 'none' : '1.5px solid #f0f0f8', borderRadius: 16, padding: '2rem', position: 'relative', boxShadow: popular ? '0 20px 50px rgba(83,74,183,0.25)' : 'none', transition: 'all 0.25s' }}>
              {popular && <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: '#1a1a2e', color: '#fff', padding: '4px 18px', borderRadius: 20, fontSize: 11, fontWeight: 700, letterSpacing: 1, whiteSpace: 'nowrap' }}>MOST POPULAR</div>}
              ...
              {features.map(f => (
                <div key={f} style={{ display: 'flex', gap: 10, marginBottom: 11, alignItems: 'center' }}>
                  <span style={{ width: 18, height: 18, background: popular ? 'rgba(255,255,255,0.2)' : '#EEEDFE', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: popular ? '#fff' : '#534AB7', flexShrink: 0, fontWeight: 700 }}>✓</span>
                  <span style={{ fontSize: 13.5, color: popular ? 'rgba(255,255,255,0.85)' : '#6b6b8a' }}>{f}</span>
                </div>
              ))}
              <button onClick={() => navigate('/auth')} className="btn-hover" style={{ width: '100%', marginTop: '1.75rem', padding: '13px', background: popular ? '#fff' : '#534AB7', color: popular ? '#534AB7' : '#fff', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', transition: 'all 0.25s' }}>
                {cta}
              </button>
            </div>
          ))}
      ...
      <section style={{ padding: '6rem 2rem', textAlign: 'center', background: '#534AB7', position: 'relative', overflow: 'hidden' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: 16, marginBottom: '2.5rem', fontWeight: 400 }}>Join hundreds of developers and agencies using IA Audit Pro</p>
        <button onClick={() => navigate('/auth')} style={{ background: '#fff', color: '#534AB7', border: 'none', padding: '16px 44px', borderRadius: 10, fontSize: 16, fontWeight: 800, cursor: 'pointer', boxShadow: '0 8px 30px rgba(0,0,0,0.15)', transition: 'all 0.25s' }} onMouseOver={e => e.currentTarget.style.transform='translateY(-2px)'} onMouseOut={e => e.currentTarget.style.transform='translateY(0)'}>
          Start Free Today
        </button>
      ...
      <footer style={{ borderTop: '1px solid #f0f0f8', padding: '2.5rem 2rem', textAlign: 'center', color: '#b0b0c8', fontSize: 13, background: '#fff' }}>
        ...
        <div style={{ display: 'flex', gap: 20, justifyContent: 'center' }}>
          <a href="#" style={{ color: '#b0b0c8', textDecoration: 'none', fontSize: 13 }} onMouseOver={e => e.target.style.color='#534AB7'} onMouseOut={e => e.target.style.color='#b0b0c8'}>Privacy Policy</a>
          <a href="#" style={{ color: '#b0b0c8', textDecoration: 'none', fontSize: 13 }} onMouseOver={e => e.target.style.color='#534AB7'} onMouseOut={e => e.target.style.color='#b0b0c8'}>Terms of Service</a>
          <a href="#" style={{ color: '#b0b0c8', textDecoration: 'none', fontSize: 13 }} onMouseOver={e => e.target.style.color='#534AB7'} onMouseOut={e => e.target.style.color='#b0b0c8'}>Contact</a>
        </div>
      </footer>

      ...
      <div className="layout">
        ...
        <main className="main">
          ...
          <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
            ...
            <h2 style={{ fontSize: 20, fontWeight: 700, marginBottom: '0.75rem' }}>Pro Feature</h2>
            <p style={{ color: 'var(--muted)', fontSize: 14, marginBottom: '1.5rem' }}>
              Competitor Analysis sirf Pro aur Agency users ke liye available hai.
            </p>
            <button className="btn btn-primary" onClick={handleUpgrade} style={{ padding: '12px 32px', fontSize: 15 }}>
          ...
          <div>
            <div className="card" style={{ marginBottom: '1rem', background: myOverall >= compOverall ? '#EAF3DE' : '#FCEBEB', border: `1px solid ${myOverall >= compOverall ? '#3B6D11' : '#A32D2D'}` }}>
              <p style={{ fontWeight: 700, fontSize: 15, color: myOverall >= compOverall ? '#3B6D11' : '#A32D2D', textAlign: 'center' }}>
                {myOverall >= compOverall ? `Your website is better! (${myOverall} vs ${compOverall})` : `Competitor is ahead! (${compOverall} vs ${myOverall})`}
              </p>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
              {[
                { label: 'My Site', url: result.my_site?.url || myUrl, scores: result.my_site?.scores || {}, strengths: result.my_site?.strengths || [], weaknesses: result.my_site?.weaknesses || [] },
                { label: 'Competitor', url: result.competitor?.url || compUrl, scores: result.competitor?.scores || {}, strengths: result.competitor?.strengths || [], weaknesses: result.competitor?.weaknesses || [] }
              ].map((site, idx) => (
                <div key={idx} className="card">
                  <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: '0.5rem', color: idx === 0 ? 'var(--accent)' : 'var(--red)' }}>{site.label}</h3>
            ...
            {result.verdict && (
              <div className="card" style={{ marginBottom: '1rem' }}>
                <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 8, color: 'var(--accent)' }}>Verdict</h3>
                <p style={{ fontSize: 13, lineHeight: 1.7, color: 'var(--muted)' }}>{result.verdict}</p>
              </div>
            )}
              ...
              <div className="card">
                ...
                {result.recommendations.map((r, i) => (
                  <div key={i} style={{ display: 'flex', gap: 10, marginBottom: 8 }}>
                    <span style={{ background: 'var(--accent)', color: '#000', borderRadius: '50%', width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, flexShrink: 0 }}>{i + 1}</span>
